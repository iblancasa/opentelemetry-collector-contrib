...
type dimensionList struct {
	nameDimensions []utilattri.Dimension
...
type connectorImp struct {
	...
	eDimensions dimensionList

	// Calls dimensions to add to the events metric.
	callsDimensions dimensionList
...
func newDimensions(cfgDims []Dimension) (dimensionList, error) {
	...
	for i := range cfgDims {
		if cfgDims[i].Glob != "" {
			...
			dims.globDimensions = append(dims.globDimensions, g)
			continue
		}
		dim := utilattri.Dimension{Name: cfgDims[i].Name}
		if cfgDims[i].Default != nil {
			val := pcommon.NewValueStr(*cfgDims[i].Default)
			dim.Value = &val
		}
		dims.nameDimensions = append(dims.nameDimensions, dim)
	}
	return dims, nil
}
...
func newConnector(logger *zap.Logger, config component.Config, clock clockwork.Clock, instanceID string) (*connectorImp, error) {
	...
	dimensions, err := newDimensions(cfg.Dimensions)
	if err != nil {
		return nil, err
	}
	callsDimensions, err := newDimensions(cfg.CallsDimensions)
	if err != nil {
		return nil, err
	}
	eDimensions, err := newDimensions(cfg.Events.Dimensions)
	if err != nil {
		return nil, err
	}
	durationDimensions, err := newDimensions(cfg.Histogram.Dimensions)
	if err != nil {
		return nil, err
	}
...
func initSelfTimeHistogramMetrics(cfg Config) metrics.HistogramMetrics {
	...
	// Reuse duration histogram settings even when duration histograms are disabled.
	selfCfg := cfg
	...
	for i := 0; i < traces.ResourceSpans().Len(); i++ {
		...
		for j := 0; j < ilsSlice.Len(); j++ {
			...
			for k := 0; k < spans.Len(); k++ {
				...
				// Prefer precomputed exclusive time from coralogixprocessor
				// (full tree, before node trim). Fall back to connector-side
				// tree computation when the attribute is absent.
				if p.selfTime.Enabled && selfTimeHistograms != nil {
					var selfTimeNS int64
					var ok bool
					if selfTimeAttr, hasAttr := span.Attributes().Get("cgx.transaction.self_time_ns"); hasAttr {
						selfTimeNS = selfTimeAttr.Int()
						ok = true
					} else if selfTimeNS, ok = selfTimeBySpanID[span.SpanID()]; ok {
						// connector-computed fallback
					}
					if ok {
						if selfTimeNS < 0 {
							selfTimeNS = 0
...
func (p *connectorImp) buildAttributes(
	...
	attr := pcommon.NewMap()
	capacity := 5 + len(dimensions.nameDimensions) + len(optionalDims.nameDimensions)
